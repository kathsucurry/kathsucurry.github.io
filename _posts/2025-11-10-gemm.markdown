...
<br /><br />
I probably have drawn the illustrations for Kernel 05 and Kernel 06 around 10 times each on papers while trying to explain the implementation to myself out loud...
</div>
<!-- END OF DIV -->
...
Once we update the offsets when shifting $A$, $B$, and $C$'s pointers, the rest of the implementation is exactly the same as the previous kernel!

Performance-wise, we don't really see much different between Kernel 04 and Kernel 05:
