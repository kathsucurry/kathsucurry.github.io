...
<div class="div-author-note">
<h4>Author's Note</h4>
As mentioned in my <a href="http://127.0.0.1:4000/cuda/2025/10/14/reduction_sum_part1.html">Reduction post</a>, I highly recommend watching <a href="https://www.nvidia.com/en-us/on-demand/session/gtcfall22-a41101/"><i>“How CUDA Programming Works” presentation by Stephen Jones</i></a>. It includes a great explanation and several examples showing how the resources required by each thread block directly affect the number of blocks that can be scheduled on each SM. Shared memory is one such resource to monitor.
</div>
<!-- END OF DIV -->
...
<br /><br />
I ended up drawing the illustrations for Kernel 05 and Kernel 06 so many times on papers while trying to explain the implementation to myself out loud (though I also personally love to draw them; pretty diagrams make me happy ❤️).
</div>
<!-- END OF DIV -->
...
| kernel 05: warp tiling | 26.19 TFLOPs/s | 82.72% |
| kernel 06: warp tiling, subdivided | 28.70 TFLOPs/s | 90.65% |
| kernel 07: transposing `As` 🆕 | 29.71 TFLOPs/s | 93.84% |
...
So far, our kernels have mainly rlied on the wmma APIs. When loading the matrices into registers using these APIs (i.e., calling `load_matrix_sync()`), we can only provide the starting address, the stride in elements between consecutive rows or columns depending on the layout, and the memory layout of the matrix. The API then implicitly determines how the data is distributed across threads in the warp.

In some situations, however, we need more flexibility over how matrix elements are assigned to individual threads. Achieving this level of flexibility requires using the low-level MMA instructions directly.
...
Note that the matrix sizes supported by wmma and mma are different. mma supports only the `m16n8k16` and `m16n8k8` shapes. In this kernel, we'll use the `m16n8k16` matrix size.
...
Since we are loading 16-bit data, we must use `.m8n8` matrix load as it is the only matrix load shape that supports 16-bit data types. Given the `.shape` parameter and the matrix size we choose then determine the `.num` parameter. In the case of choosing the `m16n8k16` matrix size, we load a `16x16` matrix from `As` into $A$ fragment registers, which is **four** `.m8n8` matrix loads. We then load a `16x8` matrix from `Bs` into $B$ fragment registers, which is **two** `.m8n8` matrix loads.

- **In some situations, not all threads are responsible for providing the values of `.p`**

Depending on the value of `.num`, every group of eight threads provides eight addresses where each address corresponds to *the start of a matrix **row*** (see the following table; we'll look into what "matrix rows" mean in the next key point). This means that if `.num` is .`x2`, only the first 16 threads in a warp provide addresses 0 - 15. Based on my experience, it seems that the instruction would just ignore whatever addresses passed by the rest of the threads.

![The addresses provided by the corresponding groups of threads in ldmatrix.](/assets/images/2025-11-10-gemm/kernel13_ldmatrix.png)
<p style="text-align: center;"><i>The addresses provided by the corresponding groups of threads in <code>ldmatrix</code>; table taken from <a href="https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-instructions-ldmatrix">PTX ISA</a></i></p>

- **The shared memory addresses the participating threads need to supply are based on the layout of the fragments**
...
![A fragment layout](/assets/images/2025-11-10-gemm/kernel13_start_addr.png)
...
- **Each thread needs to allocate and provide destination registers `r`**

Unlike how we can use the `fragment` class in wmma, we need to create our own fragment registers when using mma directly. The number of elements each thread needs to store in the $A$ and $B$ fragment registers is listed in the [PTX documentation](https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-fragment-mma-16816-float).
...
![A fragment layout](/assets/images/2025-11-10-gemm/kernel13_a_frag_layout.png)
...
![ldmatrix instruction](/assets/images/2025-11-10-gemm/kernel13_instruction.png)
...
![mma.sync instruction](/assets/images/2025-11-10-gemm/kernel13_instruction_2.png)
...
We need pay close attention to the layout of elements within the accumulator fragment here.

![Accumulator fragment layout](/assets/images/2025-11-10-gemm/kernel13_acc_frag_layout.png)
...
<div class="div-author-note">
<h4>Author's Note</h4>
You can read the <a href="https://docs.nvidia.com/cuda/inline-ptx-assembly/index.html">Using Inline PTX Assembly in CUDA</a> documentation to become familiar with the syntax and related details.
</div>
<!-- END OF DIV -->
...
- Shared memory is divided into 32 equally-sized banks, each bank can handle one 4-byte load or store operation per clock cycle. In our case, two BFloat16 elements share a single bank.
- Bank conflicts happen when multiple threads in a warp access *different* addresses within the same bank in a single instruction.
...
2. **`MBase`** --> the number of least significant bits we want to keep constant. This is useful when a group of elements must stay together, i.e., the order of the elements within the group always remain the same.
3. **`SShift`** --> the distance to shift the `YYY mask`, defined as `yyy_mask = bit_mask << (MBase + SShift)`. For simplicity, we only consider positive values of `SShift`.

Given an offset (or an index to permute), we perform the following operation:

```c++
swizzed_offset = offset ^ ((offset & yyy_mask) >> SShift);
```
...
![Swizzle toy example: original bank indices](/assets/images/2025-11-10-gemm/kernel14_example_before.png)
...
In this case, there is no need to group elements together, so we set `MBase = 0`. Each row contains 32 elements that need to be permuted while keeping the row index unchanged. That means we only need to permute the 5 least significant bits of the column index (since $2^5 =32$), so we set `BBits = 5`.

Finally, 32 elements span all 32 banks, so the value of `SShift` would be $log_{2}(32) - 0 = 5$.

Using `Mbase = 0, BBits = 5, SShift = 5` parameters results in the following updated bank indices.

![Swizzle toy example: updated bank indices](/assets/images/2025-11-10-gemm/kernel14_example_after.png)
...
If only a single 8x8 matrix is loaded, the swizzle is sufficient to eliminate bank conflicts. However, as the number of 8x8 matrices loaded per instruction increases, bank conflicts become unavoidable since the banks must be reused with the additional elements to load. Specifically, using `.num` = `.x2` results in 2-way bank conflicts even with the swizzle applied, while `.num` = `.x4` leads to 4-way bank conflicts, matching the level of conflict observed without any swizzling.

![Original bank indices the elements of As are mapped to](/assets/images/2025-11-10-gemm/kernel14_as_before.png)
...
My original goal was simply to learn from Simon's post and implement a single tensor-core-based kernel, so I did not expect to end up exploring many additional techniques such as swizzling and pipelining. In practice, wrapping my head around warp tiling (especially the subdivided variant), along with understanding how to use `mma` instructions and apply swizzling, required a significant amount of effort. I'm glad I was able to work through and cover these topics. If you notice any mistakes, issues, or have any feedback at all, I would love to hear it!

Note that I did not perform extensive parameter tuning. Instead, I experimented with a limited number of configurations and selected one that produced reasonable good performance for the chosen values of $M$, $N$, and $K$. The same configuration is unlikely to be optimal for other matrix shapes.

Overall, this post focuses on optimizing both SGEMM and HGEMM. For matrices of size $M = N = K = 4096$, we achieve approximately **95% of cuBLAS performance for SGEMM** and **101% of cuBLAS performance for HGEMM**.
