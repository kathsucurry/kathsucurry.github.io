...
| kernel 01: naive 🆕 | 1.96 TFLOPs/s | 6.22% |
...
| cuBLAS | 31.49 TFLOPs/s | 100% |
| kernel 01: naive | 1.96 TFLOPs/s | 6.22% |
| kernel 02: block tiling 🆕  | 3.46 TFLOPs/s | 10.99% |

<div class="div-example">
<h4>Kernel 02 in the Roofline Model</h4>

Recall that Kernel 02 achieves a throughput of 3.47 TFLOPs/s. The <b>total floating-point operations</b> is the same as Kernel 01, which is

$$
2MN (K + 1) FLOPs
$$

The <b>total data transferred</b>, on the other hand, has significantly decreased. Each element in $A$ is now loaded from global memory only <code>N / BLOCK_DIM</code> times, and each element of $B$ is loaded only <code>M / BLOCK_DIM</code> times. Consequently, the total global memory accesses are:
...
While Kernel 02 remains memory-bound, we can already see a shift from Kernel 01 toward the compute-bound region. To push it further, we need to increase the operational intensity even more. One effective approach is **thread coarsening** -- allowing each thread to perform more work -- which, in thise case, can further reduce global memory traffic and improve overall performance.
...
What helped me most when implementing this kernel was **decoupling the shared-memory stores from the dot-product computation and the output stores**. For instance, `TM` and `TN` are used solely in the dot-product and output stages; they play no role in how data is written to shared memory..

Additionally, during the shared-memory stage, each thread is responsible for populating `As` ($BM \times BK$) and `Bs` ($BK \times BN$). Because these tiles have different edge lengths, it's more straightforward to use **one-dimensional thread blocks** rather than two-dimensional ones.

Specifically, instead of defining a 2D block shape like this:

```c++
dim3 block_dim(BN / TN, BM / TM);
```

we can switch to a 1D layout:
...
__global__ void thread_coarsening_2d_gemm(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    ...
    for (size_t k_offset{0}; k_offset < K; k_offset += BK) {
        // Stage 1: shared-memory stores (CHANGE 4).
        ...
        // Stage 2: dot-product computation (CHANGE 5).
    ...
    // Stage 3: output stores (CHANGE 6).
    for (size_t tile_y_idx{0}; tile_y_idx < TM; ++tile_y_idx) {
        for (size_t tile_x_idx{0}; tile_x_idx < TN; ++tile_x_idx) {
...
In many cases, the shared-memory tiles are larger than the number of threads in a block. In other words, `BM x BK` and `BK x BN` may exceed the block size. When that happens, each thread must perform multiple iterations to fully populate the shared memory. These iteration counts are stored in `LOAD_ITER_M` and `LOAD_ITER_N` variables. The kernel loops over `LOAD_ITER_M` to fill `As` and over `LOAD_ITER_N` to fill `Bs`. The figure below illustrates this process. For simplicity, we assume that
...
| kernel 01: naive | 1.96 TFLOPs/s | 6.22% |
| kernel 02: block tiling | 3.46 TFLOPs/s | 10.99% |
| kernel 03: 2D thread coarsening 🆕 | 18.73 TFLOPs/s | 57.26% |

#### Kernel 04: Vectorized memory access

Global memory access is relatively slow, so it's generally recommended to **combine multiple small transfers into a single larger transfer**. This approach reduces the overhead associated with each memory access. According to the [CUDA C++ Programming Guide](https://docs.nvidia.com/cuda/cuda-c-programming-guide/#maximize-memory-throughput), global memory can be accessed using **32-, 64-, or 128-byte memory transactions**.

Up to now, all global memory loads have been performed using 32-byte transactions. In Kernel 3, for example, if we inspect the line where elements from `A` are loaded in [Godbolt](https://godbolt.org/z/nrcqdPvT3), we see the PTX instruction: `ld.global.f32`. The goal of the optimization in this kernel is to **reduce the number of global memory transfers** by merging them into **128-byte transactions**, which improves memory throughput. To do that, we use the built-in vector type `float4`, which packs four consecutive `float` elements into a single 16-byte vector.
...
__global__ void vectorize_gemm(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    ...
    for (size_t k_offset{0}; k_offset < K; k_offset += BK) {
        // Stage 1: shared-memory stores (CHANGE 3: load 4 elements at once using float4).
        for (size_t iter_m{0}; iter_m < LOAD_ITER_M; ++iter_m) {
        ...
        // Stage 2: dot-product computation (NO CHANGE).
        ...
    }

    // Stage 3: output stores (CHANGE 4: store 4 elements at once using float4).
    for (size_t tile_y_idx{0}; tile_y_idx < TM; ++tile_y_idx) {
        for (size_t tile_x_idx{0}; tile_x_idx < TN; tile_x_idx += 4) {
            ...
            reinterpret_cast<float4 *>(&C[cell_row_idx * N + cell_col_idx])[0] = tmp;
        }
    }
}
```

The updated PTX instructions are available in [this Godbolt link](https://godbolt.org/z/sE5b6vsT3). Notably, the global memory load/store operations now use `ld.global.v4.u32` and`st.global.v4.f32`, while the shared memory stores use `st.shared.v4.u32`, confirming that the kernel performs 128-byte memory transfers.

**Iterations during shared-memory stores**

Note that if previously `LOAD_ITER_M` is defined as `(BK x BM) / BLOCK_DIM`, we now have the following:
```c++
size_t const LOAD_ITER_M{(BK * BM) / (4 * blockDim.x)};
```
since each thread now loads from global memory $A$ 4 elements at once in each iteration. The same applies to `LOAD_ITER_N`.

![image Shared-memory stores in Kernel 04](/assets/images/2025-11-10-gemm/kernel4_sm_store.png)
<p style="text-align: center;"><i>An example of shared-memory stores in Kernel 04.</i></p>
...
| kernel 02: block tiling | 3.46 TFLOPs/s | 10.99% |
| kernel 03: 2D thread coarsening | 18.73 TFLOPs/s | 57.26% |
| kernel 04: vectorize memory access 🆕 | 20.54 TFLOPs/s | 62.79% |
