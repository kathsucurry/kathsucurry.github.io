...
6. [**Half-precision GEMM implementation**](#6-half-precision-gemm-implementations)
    ...
    - [**Kernel 10: Tensor cores + asynchronous gmem loads**](#kernel-10-tensor-cores--asynchronous-gmem-loads)
    - [**Kernel 11: Tensor cores + double buffering**](#kernel-11-tensor-cores--double-buffering)
    - [**Kernel 12: Tensor cores + two-level pipeline**](#kernel-12-tensor-cores--two-level-pipeline)
...
Although not shown here, using the code above should already yield some improvements. We will isolate and examine this improvement in the half-precision GEMM implementations.
...
![The pipeline of the previous kernels](/assets/images/2025-11-10-gemm/kernel9_intro_prev.png)
...
![The new pipeline in Kernel 09](/assets/images/2025-11-10-gemm/kernel9_intro_now.png)
...
<br /><br />
We will examine these points in more details later, but essentially, tensor cores operate on matrices with predetermined sizes and data types for each matrix, depending on the instruction (<code>wmma</code> or <code>mma</code>) being used.
</div>
<!-- END OF DIV -->
...
among floating-point data types, there is currently no support for using FP32 for both the multiplicands and the accumulator. As a result, we will focus on half-precision GEMM implementations in the rest of the post.
...
| kernel 05: warp tiling | 26.19 | 26.81 | 32.74% |

While cuBLAS performance shoots up to 88.81 TFLOPs/s for HGEMM, kernels up to Kernel 05 show performance similar to SGEMM, so I stopped there and shifted focus to tensor cores.

The kernels using tensor cores are as follows.

1. [**Kernel 09: Tensor cores (wmma API)**](#kernel-09-tensor-cores-wmma-api)
2. [**Kernel 10: Tensor cores + asynchronous gmem loads**](#kernel-10-tensor-cores--asynchronous-gmem-loads)
3. [**Kernel 11: Tensor cores + double buffering**](#kernel-11-tensor-cores--double-buffering)
4. [**Kernel 12: Tensor cores + two-level pipeline**](#kernel-12-tensor-cores--two-level-pipeline)
...
#### Kernel 10: Tensor cores + asynchronous gmem loads

#### Kernel 11: Tensor cores + double buffering

#### Kernel 12: Tensor cores + two-level pipeline
